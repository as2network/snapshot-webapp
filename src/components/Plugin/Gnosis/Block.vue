<template>
  <Block title="Gnosis Impact" v-if="choices.length > 1">
    ...
    <div class="mb-1" style="padding-top: 12px">
      ...
      <a :href="getMarketUrl(this.baseProductMarketMaker)" target="_blank" class="float-right">
    ...
    <div>
      <div class="mb-1">
        ...
        <a :href="getMarketUrl(this.quoteProductMarketMaker)" target="_blank" class="float-right">
          <Icon name="external-link" class="ml-1" />
        </a>
      </div>
    </div>
  ...
  <Block title="Gnosis Impact" v-else>
    <div>Two choices are required for this plugin.</div>
  </Block>
</template>
...
export default {
  ...
  async created() {
    ...
    this.predictPriceImpact =
      ((this.priceFirstOption - this.priceSecondOption) / this.priceSecondOption) * 100;
  },
  methods: {
    ...
    getTokenPrice(outcomeIndex) {
      return (
        ...
        (parseFloat(this.baseProductMarketMaker.outcomeTokenMarginalPrices[outcomeIndex]) /
