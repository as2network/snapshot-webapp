  ...
  getSpaces: async ({ commit }) => {
    ...
    let spaces: any = {
      as2network: {
        ...
        symbol: 'CAKEVOTE',
        skin: 'as2network',
        domain: 'voting.as2networkswap.finance',
        strategies: [
          {
            name: 'as2network',
            params: {
    ...
    try {
      ...
      result.results = {
        totalVotes: result.proposal.msg.payload.choices.map(
          (choice, i) =>
            Object.values(result.votes).filter((vote: any) => vote.msg.payload.choice === i + 1)
              .length,
        ),
        ...
        totalVotesBalances: Object.values(result.votes).reduce((a, b: any) => a + b.balance, 0),
      };
